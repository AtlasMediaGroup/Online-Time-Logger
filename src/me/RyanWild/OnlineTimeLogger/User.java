...
public class User
{
  ...
  public void backUp() throws IOException {
    if (OnlineTimeLogger.instance.getConfig().getBoolean("show-messages")) OnlineTimeLogger.instance.getLogger().info("Backing up " + this.username);
  ...
  public boolean needBackUP() {
    if (!OnlineTimeLogger.instance.getConfig().getBoolean("backup-enabled")) return false;
    Player player = OnlineTimeLogger.instance.getServer().getPlayerExact(this.username);
    if (player != null) {
      if ((OnlineTimeLogger.instance.getConfig().getInt("backup-type") == 0) && (!OnlineTimeLogger.instance.getServer().getPlayerExact(this.username).hasPermission("onlinetimelogger.backup"))) return false;
      if ((OnlineTimeLogger.instance.getConfig().getInt("backup-type") == 1) && (OnlineTimeLogger.instance.getServer().getPlayerExact(this.username).hasPermission("onlinetimelogger.backup"))) return false;
    }
